...
public function moveActual($ndx){
        ...
        if($current->contains(self::PIT,self::HAPPY))
            return self::FELL;

        if($current->contains(self::WUMPUS,self::HAPPY))
            return self::EATEN;

    if($current->contains(self::BIRDS,self::HAPPY))
...
public function transport(){
    ...
    public function shoot($ndx) {
        ...
        $current = $this->getRoom($ndx);
        $sroom2= $current->getNeighbors()[rand(0,2)];
        ...
        if($sroom2->contains(self::WUMPUS,self::HAPPY))
        {
            return true;
        }
...
public function smellWumpus(){
        ...
        foreach($neighbors as $first )
        {
            if($first->contains(self::WUMPUS,self::HAPPY))
                return true;
        }

    foreach($neighbors as $first )
    {
     foreach($first->getNeighbors() as $second)
         if($second->contains(self::WUMPUS,self::HAPPY))
         {
             return true;
         }
    }
...
public function feelDraft(){
    ...
    foreach($neighbors as $first )
    {
        if($first->contains(self::PIT,self::HAPPY))
            return true;
    }
    return false;
}
...
public function hearBirds(){
    ...
    foreach($neighbors as $first )
    {
        if($first->contains(self::BIRDS,self::HAPPY))
            return true;
    }
    return false;
